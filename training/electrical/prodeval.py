...
---------------------
The acceptance sweep explores ``decode_floor`` × ``unknown_floor``, which is
35 configurations in the default grid. Re-running inference for each would mean
35 full passes over the validation split. It is not necessary:
...
have produced, at 1/35th of the cost.
...
def sweep(gts: Sequence[Mapping], cache: CandidateCache,
          ...
          iou_thr: float = em.DEFAULT_IOU,
          log=None) -> dict:
    """Evaluate the full ``decode_floor`` × ``unknown_floor`` grid.
    ...
    if log:
        log(f"  best: decode={best['decode_floor']} "
            f"unknown={best['unknown_floor']} score={best['score']:.4f}")
